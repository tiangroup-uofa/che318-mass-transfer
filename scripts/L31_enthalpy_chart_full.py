...
def _(mo):
    ...
    """).batch(
        ...
        TG1=mo.ui.number(step=0.1, value=30),
...
def _(
    ...
    if setup_value["switch_tg"]:
        ax = display_tg_solution(ax, hLa=setup_value["hLa"], kGaP=setup_value["kGaP"], 
                                 op_line=_op_line, eq_line=_eq_line, T_G1=setup_value["TG1"], 
                                 L=setup_value["L"], G=setup_value["G"])
...
def _(
    ...
    def display_operating_line(ax, T_L1, H_y1, T_L2, L, G, eq_line):
        ...
        T_range = np.linspace(T_L1, T_L2, 100)
    ...
    def display_tg_solution(ax, hLa, kGaP, op_line, eq_line, T_G1, L, G):
        """Integrate the T_G bulk gas temperature from the integral equation
        """
        T_G, H_y = integrate_TG_profile(
            hLa=hLa,
            kGaP=kGaP,
            T_G1=T_G1,
            op_line=op_line,
        ...
        ax.plot((T_G[0], T_G[-1]), (H_y[0], H_y[-1]),
            "o",
            color="tab:blue",
            markersize=7,
        )
...
def _(UnivariateSpline, fsolve, np):
    ...
    def integrate_TG_profile(hLa, kGaP, T_G1, op_line, eq_line, L, G):
        """Integrate the ODE so that
        ...
        T_G = np.empty_like(T_L)
        T_G[0] = T_G1

        for j in range(len(T_L) - 1):
            dT_L = T_L[j + 1] - T_L[j]
            denom = H_yi[j] - H_y[j]
            if np.isclose(denom, 0.0):
                T_G[j + 1] = T_G[j]
                continue
            dT_G_dT_L = (L * cL / G) * (T_i[j] - T_G[j]) / denom
            T_G[j + 1] = T_G[j] + dT_G_dT_L * dT_L
